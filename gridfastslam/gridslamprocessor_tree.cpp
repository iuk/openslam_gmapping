...
void GridSlamProcessor::resetTree(){
  // don't calls this function directly, use updateTreeWeights(..) !
    ...
    for (ParticleVector::iterator it=m_particles.begin(); it!=m_particles.end(); it++)
    {
        ...
        while (n)   // 从当前粒子一直往父级循环
        {
            n->accWeight=0;     // 粒子 accWeight visitCouner 清零
			n->visitCounter=0;
			n=n->parent;
		}
	}
...
double GridSlamProcessor::propagateWeights(){
  // don't calls this function directly, use updateTreeWeights(..) !
    ...
    std::vector<double>::iterator w=m_weights.begin();  //粒子结构体内部就有权重项呀。
    // 为什么还搞一个 m_weights vector向量
